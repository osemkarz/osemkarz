    ...
    <div style={{ minHeight:'100vh', background:C.bg, fontFamily:"'DM Sans',sans-serif" }}>
      ...
      <div style={{ maxWidth:800, margin:'0 auto', padding:'28px 16px' }}>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
          {LEKCJE.map((l,i) => {
            const noPage = l.href==='#'
            const isH = hov===i && !noPage
            return (
              <div key={i} onClick={()=>!noPage&&router.push(l.href)} onMouseEnter={()=>setHov(i)} onMouseLeave={()=>setHov(null)}
                style={{ background:l.isTest?C.navy:C.white, borderRadius:14, padding:'16px 20px', display:'flex', alignItems:'center', gap:16, cursor:noPage?'default':'pointer', transform:isH?'translateY(-1px)':'none', boxShadow:isH?'0 4px 16px rgba(0,0,0,0.08)':'none', transition:'all 0.15s', border:`1px solid ${l.isTest?'rgba(255,255,255,0.1)':C.border}` }}>
                <div style={{ width:44, height:44, borderRadius:l.isTest?'50%':10, flexShrink:0, background:l.isTest?'rgba(255,255,255,0.1)':'#F0F0F0', display:'flex', alignItems:'center', justifyContent:'center', fontSize:l.isTest?18:15, fontWeight:700, color:l.isTest?'#fff':C.text3 }}>
                  {l.isTest?'🏆':l.n}
                </div>
                <div style={{ flex:1, minWidth:0 }}>
                  <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:4, flexWrap:'wrap' }}>
                    <span style={{ fontSize:15, fontWeight:600, color:l.isTest?'#fff':C.text }}>{l.title}</span>
                    {!noPage&&!l.isTest&&<span style={{ fontSize:11, background:'rgba(245,84,30,0.12)', color:C.accent, padding:'2px 8px', borderRadius:10, fontWeight:500 }}>Dostępna</span>}
                    {noPage&&!l.isTest&&<span style={{ fontSize:11, background:'#F3F4F6', color:'#9CA3AF', padding:'2px 8px', borderRadius:10 }}>Wkrótce</span>}
