...
const LEKCJE_DEFAULT = [
  { n: 1, title: 'Równania liniowe z jedną niewiadomą',           desc: 'Typy równań, rozwiązywanie krok po kroku, nawiasy, sprawdzanie',        czas: '15 min', xp: 150, href: '/kurs/rownania-liniowe',   status: 'done'   },
  { n: 2, title: 'Układy równań z dwiema niewiadomymi',            desc: 'Metoda podstawiania, metoda dodawania, interpretacja graficzna',         czas: '15 min', xp: 180, href: '/kurs/uklady-rownan',     status: 'done'   },
  { n: 3, title: 'Nierówności liniowe',                            desc: 'Rozwiązywanie, zasada znaku ⚠️, oś liczbowa, zapis przedziałowy',        czas: '15 min', xp: 180, href: '/kurs/nierownosci',       status: 'active' },
  { n: 4, title: 'Zadania tekstowe z równaniami i nierównościami', desc: 'Liczby, wiek, pieniądze, ruch, mieszaniny — strategie CKE',             czas: '14 min', xp: 200, href: '/kurs/zadania-tekstowe',   status: 'locked' },
  { n: 5, title: 'Równania i nierówności w geometrii',             desc: 'Obwody, pola, twierdzenie Pitagorasa z równaniem, kąty',                czas: '12 min', xp: 180, href: '/kurs/rownania-geometria', status: 'locked' },
  { n: 6, title: '🏆 Sprawdzian działu',                           desc: '20 zadań diagnostycznych · Analiza błędów · Raport Maxa',              czas: '30 min', xp: 400, href: '/kurs/sprawdzian-3',       status: 'locked', isTest: true },
]
...
export default function Dzial3Page() {
  ...
  useEffect(() => {
    try { const s = localStorage.getItem('osemkarz_dzial3'); if (s) setLekcje(JSON.parse(s)) } catch {}
  }, [])
  ...
  const handleReset = () => {
    ...
    setLekcje(r)
    try { localStorage.setItem('osemkarz_dzial3', JSON.stringify(r)) } catch {}
    setShowReset(false)
    setToast(true)
    ...
    <div style={{ minHeight:'100vh', background:C.bg, fontFamily:"'DM Sans',sans-serif" }}>
      ...
      {showReset && (
        <div onClick={() => setShowReset(false)} style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.5)', zIndex:100, display:'flex', alignItems:'center', justifyContent:'center', padding:20 }}>
          <div onClick={e => e.stopPropagation()} style={{ background:C.white, borderRadius:16, padding:28, maxWidth:400, width:'100%', boxShadow:'0 20px 60px rgba(0,0,0,0.3)' }}>
            ...
            <p style={{ fontSize:14, color:C.text2, lineHeight:1.7, textAlign:'center', marginBottom:24 }}>Wszystkie ukończone lekcje w tym dziale zostaną odznaczone. Zdobyte XP i odznaki pozostają.</p>
      ...
      {toast && (
        <div style={{ position:'fixed', bottom:24, left:'50%', transform:'translateX(-50%)', background:C.navy, color:'#fff', padding:'12px 24px', borderRadius:10, fontSize:13, fontWeight:500, zIndex:50, boxShadow:'0 8px 24px rgba(0,0,0,0.3)' }}>
          ✅ Postępy działu 3 zostały zresetowane
        </div>
      )}
      ...
      <nav style={{ background:C.navy, padding:'0 24px', borderBottom:'1px solid rgba(255,255,255,0.07)', position:'sticky', top:0, zIndex:10 }}>
        <div style={{ maxWidth:800, margin:'0 auto', display:'flex', alignItems:'center', gap:12, height:54 }}>
          ...
          <span style={{ fontSize:13, color:'#fff', fontWeight:500 }}>Dział 3 — Równania i nierówności</span>
      ...
      <div style={{ maxWidth:800, margin:'0 auto', padding:'28px 16px' }}>
        ...
        <div style={{ background:C.navy, borderRadius:16, padding:'28px 32px', marginBottom:20, position:'relative', overflow:'hidden' }}>
          ...
          <div style={{ fontFamily:'Fraunces,serif', fontSize:26, fontWeight:900, color:'#fff', marginBottom:8 }}>Równania i nierówności</div>
          <div style={{ fontSize:14, color:'rgba(255,255,255,0.5)', marginBottom:20, lineHeight:1.6 }}>Jeden z kluczowych działów — zwykle 3–4 zadania w arkuszu CKE.</div>
          ...
          <div style={{ display:'flex', gap:20 }}>
            {[['5','lekcji'],['1','sprawdzian'],['~75 min','łącznie'],['CKE','egzamin']].map(([v,l],i) => (
        ...
        <div style={{ background:C.white, borderRadius:12, border:`1px solid ${C.border}`, padding:'16px 20px', marginTop:14, display:'flex', gap:14, alignItems:'flex-start' }}>
          ...
          <div style={{ fontSize:13, color:C.text2, lineHeight:1.7 }}><strong style={{ color:C.text }}>Każda lekcja:</strong> Teoria (5 sekcji) → Quiz (5 pytań + "Dlaczego?") → Fiszki spaced repetition → Kartkówka (trening lub egzamin z timerem) → Raport Maxa. Sprawdzian otwiera się po ukończeniu wszystkich 5 lekcji.</div>
        </div>
        ...
        <div style={{ marginTop:14, padding:'16px 20px', background:'#FFF5F5', borderRadius:12, border:'1px solid #FED7D7', display:'flex', alignItems:'center', justifyContent:'space-between', gap:16 }}>
          <div>
            <div style={{ fontSize:13, fontWeight:500, color:'#9B2C2C', marginBottom:2 }}>Resetuj postępy działu 3</div>
        ...
        <div style={{ display:'flex', justifyContent:'space-between', marginTop:14, gap:10 }}>
          ...
          <Link href="/kurs" style={{ padding:'10px 18px', background:C.bg, border:`1px solid ${C.border}`, borderRadius:10, fontSize:13, color:C.text3, textDecoration:'none' }}>Dział 4: Funkcje →</Link>
        </div>
      </div>
    </div>
  )
